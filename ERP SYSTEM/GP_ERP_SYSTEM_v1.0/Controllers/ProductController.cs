...
namespace GP_ERP_SYSTEM_v1._0.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetProductById(int id)
        {
            ...
            try
            {
                var product = await _unitOfWork.Product.FindAsync(p => p.ProductId == id, new List<string>() { "Category" });
        ...
        public async Task<IActionResult> AddNewProduct([FromBody] AddProductDTO addProductDto)
        {
            ...
            if (!ValidateCategoryId(addProductDto.CategoryId))
                return BadRequest(new ErrorApiResponse(400, "Invalid Category Id is Sent."));

            try
            {
                _unitOfWork.Product.InsertAsync(_mapper.Map<TbProduct>(addProductDto));
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateProduct(int id, [FromBody] AddProductDTO addProductDto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                _mapper.Map(addProductDto, productToUpdate);
