...
//       `https://localhost:5001/api/AddNewEmployee`,
//       empData,config
//     );
//     navigate("/employee");
...
function NewEmployee({ inputs, title, logOut }) {
  ...
  const [emp, setEmp] = useState({
    ...
    attendenceTime: new Date().toISOString(),
    holidays: new Date().toISOString(),
    employeeSalary: 0,
    hrid: 0
  });
  ...
  async function sendData() {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error!",
        text: "Failed to add employee. Please check the data and try again.",
