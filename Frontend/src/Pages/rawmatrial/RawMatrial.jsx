...
export default function RawMatrial({logOut}) {
  ...
  const {handleUpdate,getRawMaterialById} = useContext (RawMatrialContext);
...
{
  e.preventDefault();
 await handleUpdate(materialId,rawdata);
  Swal.fire({
    icon: 'success',
    title: 'Updated!',
...
async function getRawmatrial ()
{
 ...
 const rawmatrial = await getRawMaterialById(materialId);
 console.log('====================================');
