...
//    }
//    const res = await axios.post (`https://localhost:5001/api/AddNewProduct`,newData)
//    navigate("/products");
...
function NewProduct({ inputs, title, logOut }) {
  ...
  async function sendData() {
    ...
    // الحصول على التوكن من localStorage أو أي مكان آخر
    const token = localStorage.getItem("token"); // تأكد من أن التوكن مخزن بشكل صحيح
    ...
    try {
      const res = await axios.post(
        `https://localhost:5001/api/AddNewProduct`,
        newData,
        config
      );
      navigate("/products");
      ...
      Swal.fire({
        icon: "error",
        title: "Error!",
        text: "Failed to add product. Please check the data and try again.",
