...
console.log(template);
  ...
  async function sendData() {
    ...
    const res = await axios.post(
      `https://localhost:5001/api/FmsAddTemplateAccount?tempID=${tempId}&accID=${Template.accId}`,
      TemplateData
    );
        navigate("/template");
  ...
  async function getAllAccountCategories() {
    const accountObject = await axios.get(
      `https://localhost:5001/api/FmsGetTemplateAccounts?tempID=${tempId}`
    );
  ...
  async function addStatement() {
    const state = await axios.post(
      `https://localhost:5001/api/FmsAddStatement?templateID=${tempId}`
    );
    Swal.fire({
      icon: "success",
      title: "Added!",
  ...
  async function getAllTemplates() {
    const response = await axios.get(
      `https://localhost:5001/api/FmsGetAllAccounts`
    );
