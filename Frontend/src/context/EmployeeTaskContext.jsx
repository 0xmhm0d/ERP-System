...
export function EmployeeTaskContextProvider({children})
{
    ...
    {
       const allhr = await axios.get(`https://localhost:5001/api/GetAllEmployeeTasks`);
...
//      console.log( allhr.data);
    ...
    {
           const hrObject = await axios.get(`https://localhost:5001/api/GetEmployeeTasksById/${id}`);
           return hrObject ;
 ...
 {
        const deletehr = await axios.delete(`https://localhost:5001/api/DeleteEmployeeTaskById?id=${id}`);
    ...
    const handleDelete = (id) => {
      ...
      {
             const updateEmployeeTaek = await axios.put(`https://localhost:5001/api/UpdateEmployeeTask/${id}`,updatedData);
