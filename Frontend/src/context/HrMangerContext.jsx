...
export function HrMangerContextProvider({children})
{
    ...
    {
       const allhr = await axios.get(`https://localhost:5001/api/GetAllHRMangers`);
...
//      console.log( allhr.data);
    ...
    {
           const hrObject = await axios.get(`https://localhost:5001/api/GetHRManagerByID/${id}`);
           return hrObject ;
 ...
 {
        const deletehr = await axios.delete(`https://localhost:5001/api/DeleletHRManagerById?id=${id}`);
    ...
    const handleDelete = (id) => {
      ...
      {
             const updateHR = await axios.put(`https://localhost:5001/api/UpdateHRManager/${id}`,updatedData);
